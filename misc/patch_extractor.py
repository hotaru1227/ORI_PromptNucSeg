...
class PatchExtractor(object):
    ...
    def __extract_valid(self, x):
        ...
        处理图的剩余部分：当滑动方向从左到右、从上到下时不适应时，翻转滑动方向，然后从右侧或底部边缘开始提取一个子图。在这种方式下，还会额外提取一个子图位于图像的右下角。    
        """
        im_h = x.shape[0]
        im_w = x.shape[1]
    ...
    def __extract_mirror(self, x):
        ...
        Return:
            a list of sub patches, each patch is same dtype as x
            计算填充量进行图像的镜像填充 → 图像边界上镜像填充确保每个提取的子图中心区域始终在原始图像内同时所有子图中心区域覆盖整个原始图像
        """
        diff_h = self.win_size[0] - self.step_size[0] #计算在垂直方向上窗口大小和步长之间的差异。
        padt = diff_h // 2 #计算垂直方向上填充的顶部（pad top）。
        padb = diff_h - padt #计算垂直方向上填充的底部（pad bottom）。

        diff_w = self.win_size[1] - self.step_size[1] #计算在水平方向上窗口大小和步长之间的差异。
        padl = diff_w // 2 # 计算水平方向上填充的左侧（pad left）。
        padr = diff_w - padl #计算水平方向上填充的右侧（pad right）。

        pad_type = "constant" if self.debug else "reflect" #根据 self.debug 的值选择填充类型。如果 self.debug 为真，选择常数填充；否则，选择镜像填充。
        x = np.lib.pad(x, ((padt, padb), (padl, padr), (0, 0)), pad_type) #H W进行填充 通道C不填充
        sub_patches = self.__extract_valid(x)  #镜像填充里面还是会调用不填充的子图提取
        return sub_patches
