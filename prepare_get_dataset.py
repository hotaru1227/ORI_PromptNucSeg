...
class __CoNSeP(__AbstractDataset):
    ...
    def load_ann(self, path, with_type=True):
        # 直接加载 numpy 数组
        ann_inst = np.load(path)  # 这会返回一个 ndarray
        # print("ann",ann_inst.shape)

        if with_type:
            ann_type = ann_inst[..., 1] # 加载对应的 type_map 文件

            # 合并类别
            ann_type[(ann_type == 3) | (ann_type == 4)] = 3
            ann_type[(ann_type == 5) | (ann_type == 6) | (ann_type == 7)] = 4
            # print("1111,",np.unique(ann_type))

            # 堆叠 inst_map 和 type_map
            ann = np.dstack([ann_inst, ann_type])
            ann = ann.astype("int32")
        else:
            ann = np.expand_dims(ann_inst, -1)
        ...
        return ann
####
class __PUMA(__AbstractDataset):
    """Defines the CoNSeP dataset as originally introduced in:
